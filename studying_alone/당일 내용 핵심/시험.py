...
# 정규화 대상 column 정의
scale_cols = ['시가', '고가', '저가', '종가', '거래량', '개인', '기관', '외국계', '연도', '월', '일']
...
# (입력_데이터)
feature_cols = ['시가', '고가', '저가', '종가', '거래량', '개인', '기관', '외국계', '연도', '월', '일']
label_cols = ['시가']
# label_cols = ['종가'] # 내일 사용
...
label_아모레 = pd.DataFrame(아모레_scale_df, columns=label_cols)
...
#---[ "feature_아모레, label_아모레"를 x, y데이터로 변환 ]--------------------------------------------------------------------------------------------------------
...
label_삼성 = pd.DataFrame(삼성_scale_df, columns=label_cols)
...
# print(아모레_x_train.shape)   # (1418, 11)
# print(아모레_x_test.shape)    # (355, 11)
# print(아모레_y_train.shape)   # (1418, 1)
# print(아모레_y_test.shape)    # (355, 1)

# print(삼성_x_train.shape)     # (1418, 11)
# print(삼성_x_test.shape)      # (355, 11)
...
아모레_x_train = 아모레_x_train.reshape(1418, 11, 1)              
아모레_x_test = 아모레_x_test.reshape(355, 11, 1)
아모레_y_train = 아모레_y_train.reshape(1418, 1)
아모레_y_test = 아모레_y_test.reshape(355, 1)

삼성_x_train = 삼성_x_train.reshape(1418, 11, 1)  
삼성_x_test = 삼성_x_test.reshape(355, 11, 1)
삼성_y_train = 삼성_y_train.reshape(1418, 1)
삼성_y_test = 삼성_y_test.reshape(355, 1)

#- - -[ 3차원으로 변환 확인 ] - - - - - - - - - - - - - - - - -

# print(아모레_x_train.shape)   # (1418, 11, 1)
# print(아모레_x_test.shape)    # (355, 11, 1)
# print(아모레_y_train.shape)   # (1418, 1)
# print(아모레_y_test.shape)    # (355, 1)

# print(삼성_x_train.shape)     # (1418, 11, 1)
# print(삼성_x_test.shape)      # (355, 11, 1)
# print(삼성_y_train.shape)     # (1418, 1)
# print(삼성_y_test.shape)      # (355, 1)

#====================================================================================

# 아모레_예측_시가 = np.array(아모레_예측_시가).reshape(1773, 1, 1)
# 삼성_예측_시가 = np.array(삼성_예측_시가).reshape(1773, 1, 1)
# print(아모레_예측_시가.shape)  #  (1773,)
# print(삼성_예측_시가.shape)   #  (1773,)

# 2-1. 모델 feature_아모레
input1 = Input(shape=(11, 1))    # print(x1_train.shape, x1_test.shape)   
...
#2-2 모델 feature_삼성
input2 = Input(shape=(11, 1))     # print(x2_train.shape, x2_test.shape)  
...
model.fit([아모레_x_train, 삼성_x_train], [아모레_y_train, 삼성_y_train], epochs=10, batch_size=100,
...
# 아모레, 삼성의 로스값 :   [0.023035552352666855, 0.01639113761484623, 0.006644414737820625]
# [[134500]]
# 아모레의 r2스코어 :  0.07199393019093159
# 삼성의 r2스코어 :  0.9992698942632625
# 걸린시간 :  6.784735679626465
