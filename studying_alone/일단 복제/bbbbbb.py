...
datasets = pd.read_csv(path + 'datasets.csv', sep='\t')   # sep='\t'
# print(datasets)
# print(datasets.shape)  # [15797 rows x 3 columns]

x = datasets['document']
y = datasets['label']

import numpy as np
import re   # re : 정규표현식의 약자
# 파이썬에 들어가 있는 표준라이브러리를 사용
# 한글 영어 텍스트를 다룰 때 필수임

# print(datasets.tail())   # 제일 최근 데이터만 출력해줌
...
#--[ 데이터 불러오기 ]-------------------------------------------
care = datasets['document']   # 리스트 하나의 리스트로 담음 ?? 
care = np.array(care)
...
from keras.preprocessing.text import Tokenizer
token = Tokenizer()
token.fit_on_texts(care)
print(token.word_index)



# x = token.texts_to_sequences(care)
# print(x) 
...
# # print(tokenizer)
# # x_x = x[1]
# tokened_title = tokenizer.tokenize(care)    # 데이터셋 통채로 불가능
# print(tokened_title)

# def preprocessing(text):
#     text = re.sub('\\\\', ' ', text)
#     return text 
# sentence =  x['content'].apply[preprocessing]

# stopwords_kr = ['하지만', '그런데', '그리고']  # 빼고 싶은 불용어 모음

# from wordcloud import WordCloud   # 사용전 pip install wordcloud로 다운 받아야함
# import matplotlib as plt
# def displayWordCloud(data = None, backgroundcolor = 'while', width=800, height=600 ):
#     wordcloud = wordcloud(
#                         font_path = 'font/NanumGothic.ttf',  # 한글의 경우 깨지지 않게 폰트 경로 지정
#                         stopwords = stopwords_kr,            # 불용어 지정
#                         background_color = backgroundcolor,  # 백그라운드 색 지정
