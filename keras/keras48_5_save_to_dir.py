...
augument_size = 20   # 4만장을 늘리겠다 ~ 라는 거임   /  이 데이터는 아래에서 60000의 데이터중 랜덤하게 정수를 뽑을 예정임 ~ 
...
x_augumented = train_datagen.flow(x_augumented, y_augumented,
                                  save_to_dir='D:/study_data/_temp',   # 증폭되는 구간이므로 flow 여기서 세이브 / 해당 경로에 복사된 4만개 사진을 저장
                                  batch_size=augument_size,
                                  shuffle=False).next()[0] 
end_time = time.time()  - start_time
print(augument_size , "개 증폭에 걸린시간 :", round(end_time, 3), "초")   # x_augumented의 실행시간 
