...
xy_train = train_datagen.flow_from_directory(   # directory : 폴더   / 즉! 폴더에서 가져오겠다! 라고 하는거임
   'd:/study_data/_data/image/brain/train/',   # 아까 만든 d드라이브에 데이터를 넣어놨는데 그걸 불러오는 것임! 
    ...
    batch_size=161,    #해당 데이터의 갯수가 120개이므로 121 이상으로 ! 해야할 거 같음! / 그냥 맘 편하게 1만개 넣는것도 방법임 ㅋ
                      # 이렇게 해줘야지 아래 fit에서 xy_train[0][0], xy_train[0][1], 부분중 앞의 [0]를 할 때 전체 데이터를 받을 수 있으므로 이과정을 해주는거임~
...
xy_test = train_datagen.flow_from_directory(   # directory : 폴더   / 즉! 폴더에서 가져오겠다! 라고 하는거임
   'd:/study_data/_data/image/brain/test/',   # 아까 만든 d드라이브에 데이터를 넣어놨는데 그걸 불러오는 것임! 
    target_size=(100, 100), 
    batch_size=121,   # 해당 데이터의 갯수가 120개이므로 121 이상으로 ! 해야할 거 같음! / 그냥 맘 편하게 1만개 넣는것도 방법임 ㅋ  
                      # 이렇게 해줘야지 아래 fit에서 xy_train[0][0], xy_train[0][1], 부분중 앞의 [0]를 할 때 전체 데이터를 받을 수 있으므로 이과정을 해주는거임~
...
##해당 프로젝트 핵심#####
###[fit을 사용해서 만들어봐 !!! ]---(위에 데이터에서  batch_size를 안나눠질만큼 최대치로 설정하고 fit에서 batch_size로 사용하면 된다 ~)=======================
...
#--------------------------------------------------------------------------------------------------------------------
##[ fit 사용해보자 ~ ]------------------------------------------------------------------------------------------------
hist = model.fit(xy_train[0][0], xy_train[0][1], epochs=30, validation_split=0.2, batch_size=100)
      # xy_train[0][0] -->  x_train으로 사용  /   xy_train[0][1]  -->  y_train으로 사용
         # xy_train[0][0]는 비정상인사람 데이터  /  xy_train[0][1]는 정상적인사람 데이터
###====================================================================================================================================================
