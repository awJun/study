...
# https://www.tensorflow.org/api_docs/python/tf/keras/utils/pad_sequences (참고)
from keras.preprocessing.sequence import pad_sequences  # <-- 패딩하면 빈 곤간에 0을 채우저ㅜㅁ 
# pad_x = pad_sequences(x, padding='pre', maxlen=5, truncating=)     # padding='pre' 앞에서부터 채우라는 명령  /  maxlen=5 최대글자는 5자로 한정하겟다
...
# from tensorflow.python.keras.layers import Dense, LSTM, Embedding # <-- 통상  input에서 많이 사용한다.  /  임베딩을해서 데이터의 값이 0과 1로 변경됨
# # Embedding단어의 상관 관계에 맞춰서 백터로 수치화를 했다.   <-- 이렇게만 알라고하심 정확한거는 알필요 없다고함.
...
# # 1갯수가 다 6개 최대 늘리고 pad_sequen에 tuncating이란거 사용 자르는 느낌 ? 자를때 앞이냐 뒤냐를 ㅎ는ㄱ ㅓ
# # 형권이가는 단어사전에 없음 알아서 처리해~ ;;
