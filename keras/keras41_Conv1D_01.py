...
# 2. 모델구성 
model = Sequential()
# model.add(LSTM(100, activation='relu' ,input_shape=(3, 1), return_sequences=False ))    # Bidirectional 뒤쪽에만 성능이 좋고 앞에는 의미가 없을 정도로 성능이 구린 현상을 방지하기 위해
                                                                                                # Bidirectional으로 랩핑(감싸주기)를 시전했음 ~   
model.add(Conv1D(10, input_shape=(3, 1)))                                                                                           # Bidirectional는 무조건 첫번째 아래에서 사용해야하는 듯 함 오류남 ㅋ ;;
model.add(Flatten())
...
# loss :  6.667924481007503e-06
# [8,9,10의 결과 [[11.000972]]
