"""
[프로젝트 설명 링크]
https://colab.research.google.com/drive/1sICgYTMakAdOLLPOTIVYdeUlhew-bOhs#scrollTo=nzVmwTNqRaIt
...
USE_CUDA = torch.cuda.is_available()                   # 쿠다 사용할 수 있는지 여부  / True or False가 들어간다. True면 cuda를 출력 아니면 cpu
DEVICE = torch.device('cuda:0' if USE_CUDA else 'cpu') # 쿠다 사용 가능하면 'cuda' 모드, 안되면 'cpu' 모드 / device id 생략 가능
print(torch.__version__, '사용DVICE:', DEVICE)         # 1.12.1+cpu 사용DVICE: cpu   <- 비정상
# data 와 model 을 GPU로 보내준다                      # 1.12.1 사용DVICE: cuda:0   <- 정상

# 1. data
x = np.array([1,2,3]) # (3, )
y = np.array([1,2,3])

# .to(DEVICE)를 사용해야 gpu를 사용할 수 있다
...
def evaluate(model, criterion, x, y): # 평가에서는 가중치 갱신할 필요가 없음
    ...
    with torch.no_grad(): # gradient 아예 안쓰겠다. 순전파만 사용해서 x에 대한 예상값만 사용 할 것임  / gradient갱신 ㅌ 가중치 갱신 x
        x_predict = model(x)
